...
After partitioning, _chunking_ rearranges the resulting document elements into manageable "chunks" in order to store data more efficiently. You can 
specify if and how Unstructured chunks those elements, based on your intended end use. 
This is especially critical for retrieval augmented generation (RAG), where LLMs typicaly have maximum 
[context window](https://simple.ai/p/tokens-and-context-windows) lengths for performing operations such as a 
...
To determine the best **Max Characters** length, see the documentation for the LLM that you want to use.
...
into each chunk that fits together within **Max Characters**. You can further control this behavior with by-title, by-page, or by-similarity chunking strategies. 
...
The following sections provides information about the available chunking strategies and their settings. 
