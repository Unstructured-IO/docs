...
3.  To create a new workflow for this job, in the **Select a Workflow or create a new one** dropdown list, click **New**. Fill in the following fields, and then click **Run**.
    ...
    *   **Destination**: The existing destination connector for the job. This determines where the processed data will be sent. If you do not have an existing destination connector, you must stop here, click **Close**, and create one. [Learn how](/platform-destination-connectors/overview).
    ...
    *   **Strategy**: Select the processing strategy for the data:
        ...
        *   **Hi Res**: The advantage of "hi\_res” is that it uses the document layout to gain additional information about document elements. We recommend using this strategy if your use case is highly sensitive to correct classifications for document elements.
