...
1. Through [Build it for me > Custom](#build-it-for-me-custom). This option enables you to fine-tune the kinds of settings that are in **Basic** and **Advanced**.
2. Through [Build it myself](#build-it-myself). This option offers a visual workflow designer with even more fine-tuning than the **Custom** option.  

#### Build it for me - Custom

1. On the sidebar, click **Workflows**.
2. Click **New Workflow**.
3. For **Workflow Name**, enter some unique name for this workflow.
4. In the **Sources** dropdown list, select your source location.
5. In the **Destinations** dropdown list, select your destination location.

   <Note>You can select multiple source and destination locations. Files will be ingested from all of the selected source locations, and the processed data will be delivered to all of the selected destination locations.</Note>

6. Click **Continue**.
7. If you want this workflow to run on a schedule, in the **Repeat run** dropdown list, select one of the scheduling options, and fill in the scheduling settings. Otherwise, select **Don't repeat**.
8. Click **Continue**.
9. In the **Strategy Focus** section, if the **Custom** option is disabled, inside , click **Notify me**, and follow the on-screen directions to complete the request.

   Unstructured will notify you when your account has enabled the **Custom** option. After you receive this notification, click 
   **Custom**, and continue with this procedure.

10. In the **Strategy** area, choose one of the following:
    ...
    - **Auto**: This strategy chooses the partitioning strategy based on detected document characteristics.

    [Learn more](/platform/partitioning).

11. In the **Image Summzarizer** drop-down list, choose one of the following:
...
12. In the **Table Summzarizer** drop-down list, choose one of the following:
...
13. In the **Connector Settings** area:

    - Check the **Include Page Breaks** box to include page breaks in the output, if the file type support it.
    - Check the **Infer Table Structure** box to extract any detected table elements in PDF files as HTML format into a `metadata` output field named `text_as_html`.
...
17. Click **Complete**.
18. If you did not set the workflow to run on a schedule, you can [run the worklow](#edit-delete-or-run-a-workflow) now.

#### Build it myself

1. On the sidebar, click **Workflows**.
2. Click **New Workflow**.
3. If the **Build it myself** option is disabled, inside **Build it myself**, click **Notify me**, and follow the on-screen directions to complete the request.

   Unstructured will notify you when your account has been enabled the **Build it myself** option. After you receive this notification, click 
   **Build it myself**, and continue with this procedure.
...
<AccordionGroup>
    <Accordion title="Partitioner node">
        ...
        - **Hi-res**: This strategy uses document layout to gain additional information about document elements. Unstructured recommends using this strategy if your use case is highly sensitive to correct classifications for document elements.
