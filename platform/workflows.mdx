...
9. In the **Strategy** area, choose one of the following:
    ...
    - **VLM**: This strategy provides the highest accuracy for parsing image and PDF files by using visual language models (VLMs). Files that are not images or PDFs are processed by using the **Hi-res** strategy and are charged at the **Hi-res** rate instead.
