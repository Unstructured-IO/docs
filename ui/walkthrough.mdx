...
4. The workflow designer appears.
   ...
   <Tip>
       ...
       collection of _nodes_ connected by _edges_ that go in only one direction. You can think of the DAG similar to a flowchart 
...
4. Under **Select VLM Model**, under **Vertex AI**, select **Gemini 2.0 Flash**.<br/>
   ...
   <Tip>
      ...
      If you aren't getting consistent results with one VLM for a particular set of files, switching over to another one might improve your results, depending on that VLM's capabilities and the sample data that it was trained on. 
   </Tip>
...
2. Between the **Partitioner** and **Destination** nodes, click the add (**+**) icon, and then click **Enrich > Image Description**.
   
   ![Adding an image description enrichment node](/img/ui/walkthrough/image-description.png)
...
4. Repeat this process to add four more nodes between the **Partitioner** and **Destination** nodes, as follows:

   a. Add a table description enrichment (click the add (**+**) icon to the right of the preceding node, and then click **Enrich > Table Description**).
   ...
   b. Add a table to HTML enrichment (click the add (**+**) icon to the right of the preceding node, and then click **Enrich > Table to HTML**).
   
      In the node's settings pane's **Details** tab, click:

      - If you choose **Standard** for **Mode**, any available choice for **Provider** (for example, **Anthropic**).
      - If you choose **Standard** for **Mode**, any available choice for **Model** (for example, **Claude Sonnet 4.5** if you chose **Anthropic** for **Provider**).
      - If you choose **Agentic** for **Mode**, a default provider and model are automatically selected for you.

      <Tip>
          The table to HTML enrichment generates an HTML representation of each detected table. This can help you to more quickly and accurately recreate the table's content elsewhere later as needed. This also provides additional context about the table's structure for your RAG apps, agents, and models. 
          The **Agentic** mode uses agentic AI to generate the table-to-HTML output, which typically provides more accurate output for complex tables than the **Standard** mode. [Learn more](/ui/enriching/table-to-html).
      </Tip>

   c. Add a named entity recognition enrichment (click the add (**+**) icon to the right of the preceding node, and then click **Enrich > NER**).
   ...
   d. Add a generative OCR enrichment (click the add (**+**) icon to the right of the preceding node, and then click **Enrich > Generative OCR**).
...
6. In the **Test output** pane, make sure that **Generative OCR (6 of 6)** is showing. If not, click the right arrow (**>**) until **Generative OCR (6 of 6)** appears, which will show the output from the last node in the workflow.
...
3. Use the visual schema builder to define the schema, based on the following JSON schema:
...
- [Remote quickstart](/ui/quickstart#remote-quickstart) - This quickstart shows you how to begin processing files and semi-structured data from remote source locations at scale, instead of just one local file at a time.
