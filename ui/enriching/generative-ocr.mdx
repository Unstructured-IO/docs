...
To produce generative OCR optimizations, in an **Enrichment** node in a workflow, select **Generative OCR**, and then choose one of the available provider (and model) combinations that are shown.
