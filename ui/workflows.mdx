...
<Warning>
    ...
    You can create and save a custom workflow that uses a local source connector or local destination connector. However, you cannot activate the workflow or run the workflow 
    either manually or on a schedule until a non-local source connector and non-local destination connector are added to the workflow.
