...
The Unstructured Platform Workflow API enables automation usage scenarios as well as for documentation, reporting, and recovery needs. 
This page provides an overview of the Unstructured Platform Workflow API.
...
To use the Unstructured Platform Workflow API, you must have:
...
- **An Unstructured API key**, created through your Unstructured account console.
  ...
  1. Sign in to the Unstructured Platform UI. [Learn how](/platform/quickstart).
...
The Unstructured Platform Workflow API is offered as part of the Unstructured Python SDK and as a set of Representational State Transfer (REST) endpoints.
...
allows you to call the Unstructured Platform Workflow API through standard Python code.
...
The Unstructured Platform Workflow API is also callable from a set of Representational State Transfer (REST) endpoints, which you can call through standard REST-enabled 
utilities, tools, programming languages, packages, and libraries. The following sections describe how to call the Unstructured Platform Workflow API with 
...
<Tip>
    You can also use the [Unstructured Platform Workflow API - Swagger UI](https://platform.unstructuredapp.io/docs) to call the REST endpoints 
...
<Warning>
    Do not use the Unstructured **Serverless API URL**, which is separate from the Unstructured Platform Workflow API URL.      
</Warning>
...
The following Unstructured SDKs, tools, and libraries do _not_ work with the Unstructured Platform Workflow API:

- The [Unstructured JavaScript/TypeScript SDK](/platform-api/partition-api/sdk-jsts)
- [Local single-file POST requests](/platform-api/partition-api/sdk-jsts) to the Unstructured Platform Partition API
- The [Unstructured open source Python library](/open-source/introducton/overview)
- The [Unstructued Ingest CLI](/ingestion/ingest-cli)
- The [Unstructured Ingest Python library](/ingestion/python-ingest)

The following Unstructured API URL is also _not_ supported: `https://api.unstructuredapp.io/general/v0/general` (the Unstructured Platform Partition API URL).

## Basics

The Unstructured Platform Workflow API enables you to work with [connectors](#connectors),
...
and deleting connectors, workflows, and jobs in the Unstructured Platform UI all use the Unstrucutured Platform Workflow API URL (`https://platform.unstructuredapp.io/api/v1`) by default. You do not need to 
