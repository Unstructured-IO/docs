...
- For the [Unstructured Platform Workflow API](/platform/api/overview), the API URL is typically `https://platform.unstructuredapp.io/api/v1`.
- For the [Unstructured Platform Partition API](/platform-api/partition-api/overview), the API URL is typically `https://api.unstructuredapp.io/general/v0/general`.

For the API key, the same API key works for both the [Unstructured Platform Workflow API](/platform/api/overview) key or [Unstructured Platform Partition API](/platform-api/partition-api/overview). This API key is in your Unstructured account dashboard. To access your dashboard:
...
For the API URL, note the value of the the Unstructured **Platform API URL** (for the Unstructured Platform Workflow API) or the Unstructured **Serverless API URL** (for the Unstructured Platform Partition API). 
