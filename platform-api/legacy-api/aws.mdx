...
- Set `UNSTRUCTURED_API_URL` to `http://`, followed by your load balancer's DNS name, followed by `/general/v0/general`.
  ...
  calling the [Unstructured Platform Partition API](/platform-api/partition-api/overview) URL as described elsewhere in the Unstructured API documentation.</Info>
