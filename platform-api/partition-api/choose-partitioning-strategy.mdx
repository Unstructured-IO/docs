...
You want to use the fastest, highest-precision, yet most cost-effective [partitioning strategy](/platform-api/partition-api/partitioning) overall, but you are not sure which one to specify for your particular files.
...
<Steps>
    ...
    <Step title="For object detection, do you know which particular high-resolution object detection model you want to use?">
        ...
        - If **Yes**, then set `--hi-res-model-name` (CLI), `hi_res_model_name` (Python), or `hiResModelName`(JavaScript/TypeScript) to the available model's name. [Learn about the available models](/platform-api/partition-api/choose-hi-res-model).
    </Step>
</Steps>
