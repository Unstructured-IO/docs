...
- For the Unstructured CLI, the Unstructured Python SDK, or the Unstructured open source library, to use this flow:
  ...
  - When using the Unstructured Python SDK or the Unstructured open source library, omit `partition_by_api` or explicitly set `parition_by_api=False`. Also omit `api_key` and `partition_endpoint`.