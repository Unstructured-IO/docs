...
This quickstart uses your local machine, with the [Unstructured Python SDK](/api-reference/api-services/sdk) installed. It preprocess a single-file source (input) file on your local machine, and it uses the Unstructured Serverless API to deliver the processed data to a destination (output) location, also on your local machine. Data is processed on Unstructured-hosted compute resources.
