...
Find the element with a `type` of `Address`, and print the element's `text` field value.
...
Find all elements with a `type` of `Title`. Of these, find the ones that have a `text` field that contains the phrase "Contact Us", and print the contents of each element's `metadata.link_urls` field.
