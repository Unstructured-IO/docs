...
<Note>
    For a hands-on walkthrough of the [Unstructured API workflow operations](/api-reference/workflow/overview) instead, 
    see the [MCP Hands-On Walkthrough for the Unstructured API workflow operations](/examplecode/tools/mcp), which offers 
