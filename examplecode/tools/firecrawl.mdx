...
[Firecrawl](https://firecrawl.dev/) is an open source tool that allows you to crawl websites and extract data from them.
...
vector embeddings, and then sends the results over into a serverless index within a Pinecone vector store. An optional step at the end shows how to use LangChain to automate RAG chat on this Pineconeindex's data.
...
Before you can start coding on your local machine, you must install Python, and you should also install a Python package and project manager to manage your project's Firecrawl, Pinecone, and other code dependencies. 
This walkthrough uses the popular Python package and project manager [uv](https://docs.astral.sh/uv/) (although `uv` is not required to use Firecrawl or Pinecone with Python). 
...
## Step 4 (Optional): Use the Pinecone index for RAG chat by using LangChain

In this optional step, you use [LangChain](https://www.langchain.com/) to perform RAG chat on the data in the Pinecone index. To 
complete this step, you will need the necessary LangChain packages and an OpenAI API key, which are described later in this step.

<Steps>
    <Step title="Install the LangChain packages">
        Use `uv` to install the necessary LangChain packages in your project by running the following command:

        ```python
        uv add langchain langchain-openai langchain-pinecone langchain-community
        ```
    </Step>
    <Step title="Get an OpenAI API key and add it to your project">
        1. [Sign up](https://auth.openai.com/create-account) or [log in](https://auth.openai.com/log-in) to your OpenAI account.
        2. [Get your OpenAI API key](https://platform.openai.com/api-keys) for your OpenAI account.
        3. Add the following line to your `.env` file, and replace `<your-openai-api-key>` with your OpenAI API key:

           ```text
           OPENAI_API_KEY=<your-openai-api-key>
           ```
    </Step>
    <Step title="Create the Python script to perform RAG chat">
        Create a file named `langchain_rag_chatbot.py` in the root of your project directory and add the following code to it:
        ...
        # Initialize the Pinecone client to check index stats and get the total count.
        pc = Pinecone(api_key=os.getenv("PINECONE_API_KEY"))
        index = pc.Index(os.getenv("PINECONE_INDEX_NAME"))
        stats = index.describe_index_stats()
        total_vectors = stats['total_vector_count']

        # Create the vector store instance from the existing serverless index.
        pcvs = PineconeVectorStore(
            index_name=os.getenv("PINECONE_INDEX_NAME"),
            embedding=FakeEmbeddings(size=3072),
            pinecone_api_key=os.getenv("PINECONE_API_KEY")
        )

        # Set up the the retrieval chain with supported search parameters.
        retrieval_qa_chat_prompt = hub.pull("langchain-ai/retrieval-qa-chat")
        retriever = pcvs.as_retriever(
            search_kwargs={
                "k": total_vectors,  # Retrieve all documents.
                "namespace": "default"  # Specify the namespace where the vectors are stored.
            }
        )
        ...
        while True:
            ...
            user_query = input("Enter your question, or enter 'q' or 'quit' to exit: ")
        ...
        - Initializes the Pinecone client to check index stats and get the total count.
        - Creates a vector store instance from the existing serverless index.
        - Sets up the the retrieval chain with supported search parameters.
        - Creates the RAG chain.
        - Runs the chatbot loop, which prompts the user for a question and then uses the RAG chain to respond.

    </Step>
    <Step title="Run the script to perform RAG chat">
        1. From the root of your project, use `uv` to run the script to perform RAG chat by running the following command:

           ```bash
           uv run langchain_rag_chatbot.py
           ```

        2. When prompted, enter a question to ask the chatbot.
        3. The chatbot will attempt to respond with an answer based on the data in the Pinecone index.
        4. You can keep asking more questions, or enter `q` or `quit` to exit the chatbot.
    </Step>
</Steps>
...
- [LangChain and Pinecone](https://docs.pinecone.io/integrations/langchain)