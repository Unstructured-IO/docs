...
<Tip>
    ...
    skip ahead to [Optional Step 3: Extend the MCP server](#optional-step-3-extend-the-mcp-server).
</Tip>
...
Unstructured offers an initial experimental MCP server implementation, with much more functionality then the MCP server that you just created in the previous Steps 1-2. 
...
<Steps>
    <Step>
        From a different working directory than the one you used in the previous Steps 1-2, use Git to clone the 
