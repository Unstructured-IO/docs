...
whenever a file upload event is detected in the specified Databricks Volume. This job uses a custom Databricks notebook to call the [Unstructured API workflow operations](/api-reference/workflow/overview) to automatically run the 
...
7. Replace the placeholders in this second cell as follows:
   ...
   - Replace `<unstructured-api-url>` with the value of the **Unstructured API workflow operations** field earlier from the requirements.
