...
An element ID is a SHA-256 hash that is computed including the element text, element's sequence number on page, page number,
and document filename. This allows the element IDs to be deterministic and unique.
