...
structure into consideration. For example, if you plan to summarize a document, you may only be interested in the narrative
...
By default, an element ID is a SHA-256 hash of the element text. This is to ensure that the ID is deterministic.
However, the ID is not guaranteed to be unique. Different elements with the same text will have the same ID, and there could
also be hash collisions. If you'd like to use UUIDs in the output instead, you can set the `unique_element_ids` parameter to `True`
when partitioning documents. This can be helpful if you'd like to use the IDs as a primary key in a database, for example.
...
|------------------------|----------------------|-------------------------------------------------------------------------------|
| `page_number`          | DOCX, PDF, PPT, XLSX | Page mumber                                                                   |
