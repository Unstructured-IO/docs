...
to process a PDF. When choosing a partitioning strategy for your files, you have to be mindful of the quality/speed
trade-off. For example, the `fast` strategy is roughly 100x faster than leading image-to-text models.
...
*   `fast`: The "fast" strategy leverages traditional NLP extraction techniques to quickly pull all the text elements. "Fast" strategy is not recommended for image-based file types.
*   `hi_res`: The "hi_res" strategy identifies the layout of the document using `detectron2`. The advantage of "hi_res" is that it uses the document layout to gain additional information about document elements. We recommend using this strategy if your use case is highly sensitive to correct classifications for document elements.
