Batch process all your records to store structured outputs in an Azure Storage account.

You will need:

import SharedAzure from '/snippets/general-shared-text/azure.mdx';
import SharedAPIKeyURL from '/snippets/general-shared-text/api-key-url.mdx'

<SharedAzure />

<SharedAPIKeyURL />

The source connector can be any of the ones supported. This example uses the local source connector:

import AzureSh from '/snippets/destination_connectors/azure_api.sh.mdx';
import AzurePyV2 from '/snippets/destination_connectors/azure_api.v2.py.mdx';
import AzurePyV1 from '/snippets/destination_connectors/azure_api.v1.py.mdx';

<CodeGroup>

  <AzureSh />

  <AzurePyV2 />

  <AzurePyV1 />

</CodeGroup>