...
- `HUBSPOT_API_TOKEN` - The access token value for the HubSpot private app, represeted by `--api-token` (CLI) or `api_token` (Python).
...
- `HUBSPOT_PROPERTIES` - Any filtering to be applied to specified object types, declared as a a dictionary of key-value pairs (Python) specifying one or more of the default 
