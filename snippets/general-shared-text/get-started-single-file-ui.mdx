...
- To download the JSON view of the results as a local JSON file, click the download icon to the left of the **Formatted** and **JSON** buttons in the **Result** pane. 
  (You cannot download the formatted view of theresults.)
...
- To have Unstructured do more&mdash;such as 
  ...
  processing larger files and semi-structured data in batchesat scale&mdash;click 
  **Edit in Workflow Editor** at the right on the title bar, or [skip over to the walkthrough](/ui/walkthrough).

  ![Switching to the workflow editor](/img/ui/single-file/workflow-editor.png)

<Icon icon="arrow-up" />&nbsp;&nbsp;[Learn how to add chunking, embeddings, and additional enrichments to your results](/ui/walkthrough).
