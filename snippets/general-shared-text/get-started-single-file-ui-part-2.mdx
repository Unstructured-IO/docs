<Warning>
    ...
    It will be a bit more difficult to complete this walkthrough unless you have already completed that quickstart.

    If you have not already done so, we recommend that you [skip back](/welcome#unstructured-ui-quickstart) and complete the quickstart first.
</Warning>
...
If the workflow editor is not visible, do the following to show it:
...
In this step, you will test the **High Res** partitioning strategy on the "Chinese Characters" and "Nash letters" PDF files that you downloaded earlier.
...
5. Some interesting portions of the output include the following:

   - The ...

6. Now try looking at the "Nash letters" PDF file's output. To do this:
...
4. Change the **Source** node to use the "Chinese Characters" PDF file, and then click **Test**.
