...
- The name of the target table in the database. To get a list of available tables in a database, you can run a Teradata SQL query such as the following, replacing `<database-name>` with the name of the target database:
  ...
  If you specify a table name and it does not exist, Unstructured creates it with the standard schema. If a table name is not specified or already exists, Unstructured creates a table called `<unstructuredautocreated>` with the following schema:
...
PRIMARY INDEX ("id");
 ```
  ...
  Here is a legacy example table schema that is compatible with Unstructured. It includes all of the required and recommended columns, as 
