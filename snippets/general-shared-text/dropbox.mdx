...
2. A Dropbox app for your Dropbox account. To create a Dropbox app, do the following:
   ...
   <Warning>
      ...
      For the [Unstructured UI](/ui/overview) or the [Unstructured API](/api/overview), or if you want to otherwise use the refresh token to replace the expired access token yourself, currently you must manually replace expired access tokens.
   </Warning>
...
8. For the [Unstructured UI](/ui/overview) or the [Unstructured API](/api/overview), or if you want to otherwise use the refresh token to replace the expired access token yourself, make the following REST API call, replacing the following placeholders:
