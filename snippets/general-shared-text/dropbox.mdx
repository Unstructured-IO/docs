...
2. A Dropbox app for your Dropbox account. To create a Dropbox app, do the following:
   ...
   h) On the app's **Settings** tab, note the value of the **App folder name** field. This will be the app folder under the `Apps` top-level folder in your Dropbox 
      account that the Dropbox app will use for access. If you change the value of **App folder name** field here, then you _must_ also have an app folder with the same name under the 
      `Apps` top-level folder in your Dropbox account. <br/>
   i) Under **OAuth 2**, next to **Generated access token**, click **Generate**. Copy the value of this access token. Unstructured recommends that you do not click **Generate** until you have completed all of the preceding steps first.<br/>
