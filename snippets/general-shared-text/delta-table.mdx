...
- One `.parquet` file per file in the source location. For example, for a file in the source location named `my-file.pdf`, an associated 
  file with the extension `.parquet` is generated in the specified path to the S3 bucket (or the specified folder within the bucket).
...
- In the source location, each set of files that is to be considered as a unit for change management purposes should be controlled by a unique, dedicated 
  ...
  multiple workflows refer to different sets of files, yet all use the same Delta Tables in S3 destination connector to write to the same path or folder, could break the associated 
  Delta table or return unexpected results.
- The set of `.parquet` files and their associated `_delta_log` folder (and its contents) that define a single Delta table should not be manually altered in place. All changes to this 
  Delta table should be made only through its associated Delta Tables in S3 destination connector. Manual alterations could break the associated Delta table or return unexpected results.
- If you need to copy or move the Delta table to a different location, such as copying or moving the Delta table from the S3 bucket into a Unity Catalog volume within a Databricks workspace, 
  you must move or copy the entire set of `.parquet` files and its associated `_delta_log` folder (and its contents) together as a unit. Note that the copied or moved Delta table will 
