...
*   <Icon icon="v"/><Icon icon="2"/>&nbsp;&nbsp;`chunking_endpoint`: If `chunk_by_api` is set to `True`, chunking requests are sent to this Unstructured API URL. By default, this URL is the Unstructured Platform Partition API URL: `https://api.unstructuredapp.io/general/v0/general`.
