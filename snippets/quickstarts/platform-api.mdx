...
- A compatible source (input) location that contains your data for Unstructured to process. [See the list of supported source types](/platform/connectors#sources). 
  This quickstart uses an Amazon S3 bucket as the source location.
...
- A compatible destination (output) location for Unstructured to put the processed data. [See the list of supported destination types](/platform/connectors#destinations). 
  For this quickstart's destination location, a different folder in the same Amazon S3 bucket as the source location is used.
...
<Steps>
    ...
    <Step title="Create and set up the S3 bucket">
        ...
        (You can use other source and destination types that are supported by Unstructured. However, they will not work with this particular quickstart.)
