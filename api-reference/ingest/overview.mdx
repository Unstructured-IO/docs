...
```mermaid
  flowchart TB
    Index[1. Ingest]-->Download
    Download[2. Download]-->Uncompress
    Uncompress[3. Uncompress]-->Partition
    Partition[4. Partition]-->Chunk
    Chunk[5. Chunk]-->Embed
    Embed[6. Embed]-->Stage
    Stage[7. Stage]-->Upload[8. Upload]
```
<Steps>
    <Step title="Index">
        Reaches out to the source location and pulls in metadata for each document to be downloaded and processed.
    ...
    <Step title="Download">
        Using the information generated from the indexer, downloads the content as files on the local file system for processing. This may require manipulation of the data to prepare it for partitioning.
    ...
    <Step title="Upload">
        Writes the local content to the destination. If none if provided, the local one will be used, which writes the final result to a location on the local filesystem. If batch uploads are needed, this will run in a single process with access to all documents. If batch uploads are not supported, all documents can be uploaded concurrently by using the asynchronous approach.
