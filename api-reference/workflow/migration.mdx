...
- Remove references to classes that end in `ConnectorConfigInput` from your code. Replace these references with a 
  typed dictionary (a [TypedDict](https://docs.pydantic.dev/latest/api/standard_library_types/#typeddict) in 
  [Pydantic](https://docs.pydantic.dev/latest/)). This typed dictionary must contain the same fields as the 
