...
- `provider`: _Required_. If the Auto partitioning strategy needs to use the VLM partitioning strategy, then use the specified VLM provider. Allowed values include `auto`, `openai`, `anthropic`, and `bedrock`. The default value is `anthropic`.
...
- `output_format`: _Required_. The format of the response. Allowed values include `text/html` and `application/json`. The default is `text/html`.
...
- `format_html`: _Required_. ... True (the default) to ...
- `unique_element_ids`: _Required_. True (the default) to assign UUIDs to element IDs, which guarantees their uniqueness. This is useful for example when using them as primary keys in a database. False to assign a SHA-256 of the element's text as its element ID.
...
- `strategy`: _Required_. The partitioning strategy to use. This field must be set to `vlm`.
- `provider`: _Required_. ... The default is `anthropic`.
- `provider_api_key`: _Optional_. ... The default is none.
- `model`: _Optional_. ... The default is `claude-3-5-sonnet-20241022`.
- `output_format`: _Required_. Allowed values include `text/html` and `application/json`. The default is `text/html`.
- `prompt`: _Optional_. The default is none.
- `format_html`: _Required_. ... The default is true.
- `unique_element_ids`: _Required_. ... The default is true.
- `is_dynamic`: _Optional_. ... The default is false.
- `allow_fast`: _Optional_. ... The default is true.
...
- `include_page_breaks: _Required_. True to include page breaks in the output if supported by the file type. The default is false.
- `pdf_infer_table_structure`: _Required_. True to ... The default is false.
...
- `xml_keep_tags`: _Required_. True to retain any XML tags in the output. False (the default) to just extract the text from any XML tags instead.
- `encoding`: _Required_. The encoding method used to decode the text input. The default is `utf-8`.
- `ocr_languages`: _Required_. A list of languages present in the input, for use in partitioning, OCR, or both. Multiple languages indicate that the text could be in any of the specified languages. The default is `[ 'eng' ]`. [See the language codes list](https://github.com/Unstructured-IO/unstructured/blob/main/unstructured/partition/common/lang.py).
- `extract_image_block_types`: _Required_. A list of the Unstructured element types for use in extracting image blocks as Base64 encoded data stored in `metadata` fields. Available values include `Image` and `Table`. The default is `[ 'Image', 'Table' ]`.
- `infer_table_structure`: _Required_. True to have any table elements extracted from a PDF to include an additional `metadata` field named `text_as_html`, containing an HTML `<table>` transformation. The default is false.
...
- `include_page_breaks: _Required_. ... The default is false.
- `pdf_infer_table_structure`: _Required_. ... The default is false.
- `exclude_elements`: _Optional_. ... The default is none.
- `xml_keep_tags`: _Required_. ... The default is false.,
- `encoding`: _Required_. ... The default is `utf-8`.
- `ocr_languages`: _Required_. The default is `[ 'eng' ]`.
- `extract_image_block_types`: _Required_. ... The default is `[ 'image', 'table' ]`.
- `infer_table_structure`: _Required_. ... The default is false.
...
- `unstructured_api_url`: _Optional_. ... The default is none.
- `unstructured_api_key`: _Optional_. ... The default is none.
...
- `new_after_n_chars`: _Optional_. ... The default is none.
- `max_characters`: _Optional_. ... The default is none. 
- `overlap`: _Optional_. ... The default is none.
- `overlap_all`: _Optional_. ... The default is false.
- `contextual_chunking_strategy`: _Optional_. ... Allowed values include `v1`. The default is none.
...
- `unstructured_api_url`: _Optional_. ... The default is none.
- `unstructured_api_key`: _Optional_. ... The default is none.
- `multipage_sections`: _Optional_. ... The default is false.
- `combine_text_under_n_chars`: _Optional_. ... The default is none.
- `include_orig_elements`: _Optional_. ... The default is false.
- `new_after_n_chars`: _Optional_. ... The default is none.
- `max_characters`: _Optional_. ... The default is none. 
- `overlap`: _Optional_. ... The default is none.
- `overlap_all`: _Optional_. ... The default is false.
...
- `overlap_all`: _Optional_. ... The default is false.
- `contextual_chunking_strategy`: _Optional_. ... Allowed values include `v1`. The default is none.
- `similarity_threshold`: _Optional_. ... The default is none.
