...
Choose your partitioning strategy wisely. For example, if you have simple PDFs that don't have images and tables, you might be able to use the `fast` strategy. Try the `fast` strategy on a few of your documents before you try using the `hi_res` strategy. [Learn more](/api-reference/partition/legacy-api/partitioning).

To speed up PDF file processing, the [Unstructured SDK for Python](/api-reference/partition/legacy-api/sdk-python) and the [Unstructured SDK for JavaScript/TypeScript](/api-reference/partition/legacy-api/sdk-jsts) provide the following parameters to help speed up processing a large PDF file:
...
[Learn more](/api-reference/partition/legacy-api/sdk-python#page-splitting).
