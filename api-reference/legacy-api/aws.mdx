...
- Set `UNSTRUCTURED_API_URL` to `http://`, followed by your load balancer's DNS name, followed by `/general/v0/general`.
  ...
  calling the [Unstructured Partition Endpoint](/api-reference/partition/legacy-api/overview) URL as described elsewhere in the Unstructured API documentation.</Info>
