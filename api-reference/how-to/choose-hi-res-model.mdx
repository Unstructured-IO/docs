...
<Steps>
    ...
    <Step title="Specify one of the following models...">
        - `detectron2_onnx` generally underperforms the following models. However, it is still accessible to maintain backwards compatibility.
        - `yolox` is also provided for backwards compatibility and originally was the replacement for `detectron2_onnx`.
        - `layout_v1.1.0` generally performs better than `yolox` at bounding box definitions and element classification. `layout_v1.1.0` is a propietary Unstructured object detection model and is used by default, as applicable, if `--hi-res-model-name` or `hi_res_model_name` is not specified.
