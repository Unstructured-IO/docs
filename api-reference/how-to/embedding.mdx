...
You can use the [Unstructured Ingest CLI](/ingestion/overview#unstructured-ingest-cli) or the [Unstructured Ingest Python library](/ingestion/overview#unstructured-ingest-python-library) to generate embeddings after the partitioning step in an ingest pipeline. You do this by 
specifying an _embedding model_ that is provided or used by an _embedding provider_. An [embedding model](https://python.langchain.com/v0.2/docs/concepts/#embedding-models) creates a complex mathematical representation 
known as a _vector_ that captures inherent properties and relationships between data. (In this case, this data is the text that is extracted by Unstructured.) 
These vector embeddings allow _large language models_ (LLMs) and _vector databases_ to more quickly and efficiently analyze and process these inherent properties and relationships. 
...
1. Choose an embedding provider that you want to use from among the following allowed providers, and note the provider's ID:
   ...
   - `octai` for [Octo AI](https://octo.ai/). [Learn more](https://octo.ai/docs/text-gen-solution/using-unstructured-io-for-embedding-documents).

2. For the following embedding providers, you can choose the model that you want to to use. If you do choose a model, note the model's name:
   ...
   - `octai`. [Choose a model](https://octo.ai/blog/supercharge-rag-performance-using-octoai-and-unstructured-embeddings/), or use the default model `thenlper/gte-large`.
...
<Icon icon="blog"/>&nbsp;&nbsp;[Understanding embedding models: make an informed choice for your RAG](https://unstructured.io/blog/understanding-embedding-models-make-an-informed-choice-for-your-rag)
