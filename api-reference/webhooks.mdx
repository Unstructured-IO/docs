...
Whenever a notification channel is created, a secret is generated for it. You can have Unstructured generate this secret's value for you, 
or you can specify this secret's value yourself. You can get this secret's value and then [verify this secret](https://docs.svix.com/receiving/verifying-payloads/how) 
to check whether the notification channel's webhook originated from Unstructured. If you suspect that the original secret was somehow 
compromised, you can rotate this secret to a new value. 
...
- Optionally, replace `<secret>` with a Base64-encoded string to serve as the secret for the notification channel. If you do not provide a secret, a random secret will be generated for you.
...
## Get the secret for a workspace-scoped notification channel

To get the secret for a notification channel that is used for all corresponding events across a workspace, 
call the `GET /notifications/channels/<channel-id>/secret` operation.
...
<AccordionGroup>
    <Accordion title="curl">
        ```bash
        curl --request 'GET' --location \
        "$UNSTRUCTURED_API_URL/notifications/channels/<channel-id>/secret" \
        --header "unstructured-api-key: $UNSTRUCTURED_API_KEY"
        ```
    </Accordion>
    <Accordion title="Postman">
...
To rotate the secret for a notification channel that is used for all corresponding events across a workspace, 
...
## Get the secret for a workflow-scoped notification channel

To get the secret for a notification channel that is used only for a specific workflow, 
call the `GET /workflows/<workflow-id>/notifications/channels/<channel-id>/secret` operation.
...
<AccordionGroup>
    <Accordion title="curl">
        ```bash
        curl --request 'GET' --location \
        "$UNSTRUCTURED_API_URL/workflows/<workflow-id>/notifications/channels/<channel-id>/secret" \
        --header "unstructured-api-key: $UNSTRUCTURED_API_KEY"
        ```
    </Accordion>
    <Accordion title="Postman">
...
To rotate the secret for a notification channel that is used only for a specific workflow, 
