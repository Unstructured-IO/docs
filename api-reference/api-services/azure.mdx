...
<Steps>
    ...
    <Step title="Post-deployment: set API environment variables">
        ...
        5. In the connected terminal, run the following command: `sudo docker images`.
        6. Note the following values:

           - `REPOSITORY` - For example, `111111111111.dkr.ecr.us-east-2.amazonaws.com/aaaa-aaa-aaaaaaaaa`
           - `TAG` - For example, `core-product-api-v1.1.11`

        5. In the connect terminal, run the following Docker container on the instance, replacing `<repository>` with the `REPOSITORY` value and `<tag>` with the `TAG` value:
           ...
           sudo docker run -d --restart unless-stopped \
           -p 80:5000 \
           111111111111.dkr.ecr.us-east-2.amazonaws.com/aaaa-aaa-aaaaaaaaa:core-product-api-v1.1.11
           ```

        6. Run the following command as needed to set each envrionment variable, as needed:
