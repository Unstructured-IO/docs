...
you can do this, which all lead to the same result, so pick your preferred method. Regardless of the method you pick, you will need:

import SharedAPIKeyURL from '/snippets/general-shared-text/api-key-url.mdx';

<SharedAPIKeyURL />

<Info>For all Free Unstructured API users, the Unstructured API URL is `https://api.unstructuredapp.io` when making a `POST` request, and a blank string when using the CLI, SDKs, or open source library.</Info>

### POST request

Supply your API URL, key, and the file to preprocess:
...
Finally, you can call the Unstructured API directly from the Unstructured open source library. Unstructured recommends this approach only for rapid local script or code prototyping or simple proofs-of-concept, not for production scenarios:
