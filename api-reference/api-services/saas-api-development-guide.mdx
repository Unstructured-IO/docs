...
description: This pages describes how to get started with the SaaS Unstructured API. Learn how to obtain an API key to use with the SaaS Unstructured API, and get started in no time.
---

## Get an API Key

The SaaS Unstructured API users receive a unique API URL to use that requires authentication via an API key.
...
Save your API key and the unique API URL securely, as you will need them to access and integrate the SaaS Unstructured API with your applications.
...
3.  Click the “Pay this invoice” button in the email and follow the instructions to enter your payment details securely on Stripe website.
...
Next, use it to call the API. Provide the API URL from the email you received, make sure to use the one **without** the
path `/general/v0/general`. Double-check that you use `https` in the URL to prevent potential errors. Use your SaaS
...
elements = partition_via_api(
  ...
  api_url="YOUR API_ URL/general/v0/general"
)
```
