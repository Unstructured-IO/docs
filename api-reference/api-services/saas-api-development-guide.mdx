...
To work with the Unstructured Serverless API by calling the Unstructured REST API POST operation, you will need to:
...
Now, use a REST API client such as `curl` to call the API, specifying where the source (input) files are to preprocess and the destination (output) where Unstructured will deliver the processed data:
...
curl -X POST $UNSTRUCTURED_API_URL \
     ...
     -F 'files=@<local/path/to/input/files>' \
     -o '<local/path/to/output/files>'
```
