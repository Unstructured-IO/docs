    ...
    ### Retries
        ...
        ```python Python
            ...
            retry_strategy = Retry(
                ...
                method_whitelist=["POST"]  # method_whitelist does not include POST by default
            )
                    ...
                    while (attempt < MAX_RETRIES) {
                        try {
                            ...
                            if ([503].includes(response.status)) {
                                throw new Error(`Retryable response status: ${response.status}`);
                            }
        ...
        ```
        ```javascript JavaScript SDK Hooks
            httpClient.addHook("response", (response, request) => {
                // configure retries on certain responses
            });
        ```
        </CodeGroup>
