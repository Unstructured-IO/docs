...
SDKs allow for easy interaction with the Unstructured API. Whether you're using the
...
## Basics
    ...
    Let's start with a simple example in which you send a pdf document to partition via the free Unstructured API:
...
## Handling the Response
    ...
    The partition response defaults to a json format. Below is an example of reading it:
...
## Page Splitting
    ...
    cause the PDF to be split page-by-page by the client before sending requests to the API. The client
    ...
    error is returned only if all requests failed or there was no PDF split.

    When using page splitting, note that chunking will not always work since chunking may happen on the
    API side. If you need to chunk, you can make a second request to the API with the returned elements.
    ...
    ```javascript JavaScript
        client.general.partition({
            ...
            splitPdfPage: true,
            // Modify splitPdfConcurrencyLevel to change the number of parallel requests
...
## Customizing the Client
    ### Configuring Retry Logic
        If a request to the API fails, the client will retry the request with an 
        exponential backoff strategy up to a maximum interval of one minute. The function keeps
        retrying until the total elapsed time exceeds `maxElapsedTime`[*](#parameter-names), which defaults to one hour.
        These defaults can be customized through the `retryConfig`[*](#parameter-names) when initializing the client:
        ...
        ```python Python
            client = UnstructuredClient(
                ...
                retry_config=RetryConfig("backoff", BackoffStrategy(
                    ...
                    max_interval=60000,
                    exponent=1.5,
                    max_elapsed_time=900000,  # 15*60*1000 = 15 minutes
                ), False),
            ...
            const unstructuredClient = new UnstructuredClient({
                ...
                retryConfig: {
                    ...
                    backoff: {
                        ...
                        maxInterval: 60000,
                        exponent: 1.5,
                        maxElapsedTime: 900000, // 15*60*1000 = 15 minutes
                    },
    ...
    ### Adding Logging
        Users have to leverage the underlying client to configure logging (requests/http_client)
...
## Parameters & examples
    ...
    #### Parameter names 
        The parameter names used in this document are the JavaScript SDK names, which follow camelCase
        convention, while the Python SDK uses snake_case. Otherwise, the API parameters are the same across all methods of accessing the Unstructured API.
