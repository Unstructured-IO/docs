...
11.  **Get the Unstructured API endpoint**:
    ...
    c.   On the **EC2 > Load balancers > (Load balancer ID)** page, copy the **DNS Name** value, which is shown as an **(A Record)** and ends with `.elb.amazonaws.com`.
         ...
         - Note: You will use this **DNS Name** to replace the `<api_url>` for the following healthcheck and data processing steps.
...
Perform a health check by running this `curl` command:
...
Data processing can be performed by using `curl` commands.
