...
*   `multipage_sections (Default: True)`: When False, in addition to section boundaries, page boundaries are also respected. Only operative for the “by\_title” strategy.
    
*   `new_after_n_chars (Default: max_characters (off))`: Cuts off new chunks once they reach a length of n characters (soft max). Defaults to max\_characters when not specified, which effectively disables any soft window. Specifying 0 for this argument causes each element to appear in a chunk by itself (although an element with text longer than max\_characters will be still be divided into two or more chunks using text-splitting).