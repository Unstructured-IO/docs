---
title: Key Concepts
description: Natural Language Processing (NLP) encompasses various tasks and methodologies. This section introduces fundamental concepts crucial for most NLP projects involving Unstructured products.
---

## Data Ingestion

Unstructured `Source Connectors` make data ingestion easy. They ensure that your data is accessible, up to date, and usable for any downstream task. If you’d like to read more on our source connectors, you can find details [here](/ingest/source-connectors/overview).
...
1.  **Data ingestion**: The first step is acquiring data from your relevant sources. We make this easy with our [source connectors](/ingest/source-connectors/overview).
    
2.  **Data preprocessing and cleaning**: Once you’ve identified and collected your data sources, removing any unnecessary artifacts within the dataset is a good practice. At Unstructured, we have various tools for data processing in our [core functionalities](/core-functionality/overview).
    
3.  **Chunking**: The next step is to break your text into digestible pieces for your LLM to consume. We provide the basic and context-aware chunking strategies. Please refer to the documentation [here](/core-functionality/chunking).
    
4.  **Embedding**: After chunking, you must convert the text into a numerical representation (vector embedding) that an LLM can understand. To use the various embedding models using Unstructured tools, please refer to [this page](/core-functionality/embedding).
